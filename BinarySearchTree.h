...
private:
    ...
    //Find the depth for tree search
    int findDepth(const Comparable &c, BinaryNode* n, int depth) const {
        if (n == nullptr) {
            // Reached a dead end. Value not in tree.
            return depth;
        }
...
public:
    ...
    int findDepth(const Comparable &c) const {
        // calls private helper function
        int depth = 0; //always starts looking at root
