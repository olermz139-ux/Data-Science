#ifndef HASHING_QUADRATICPROBING_H
#define HASHING_QUADRATICPROBING_H
...
private:
    ...
    unsigned int numItems, collisions;
...
public:
    ...
    QuadraticProbing(unsigned long tableSize) {
        ...
        numItems = 0;
        collisions = 0; //collisions are originally 0
    }
    ...
    void insert(string key, Keyable value) {
        ...
        if (!find(key)) {
            ...
            while (table[index].status == FILLED) {
                // quadratic function for quadratic probing
                index = quadCons * quadCons;
                index %= table.size();
                quadCons ++;
            }
            table[index].key = key;
