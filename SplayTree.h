...
private:
    ...
    // Helper recursive function to find a value in the tree.
    int findDepth(const Comparable &c, SplayNode* &n, int depth) {
        if (n == nullptr) {
            // Reached a dead end. Value not in tree.
            return depth;
        }
        if (c < n->value) {
            // Value is less than current node. Go to node's left child.
            return findDepth(c, n->leftChild, depth + 1);
        }
        if (n->value < c) {
            // Value is greater than current node. Go to node's right child.
            return findDepth(c, n->rightChild, depth + 1);
        }
        // If code reaches here, c == n->value. Node found!
        splay(n); //After splay the node will always be the root, but we want to record how far it travels to find node
        return depth;
    }
...
public:
    ...
    int findDepth(const Comparable &c) {
        // calls private helper function
        int depth = 0;
        return findDepth(c, root, depth);
    }
